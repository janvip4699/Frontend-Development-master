...
export default function Navbar() {
  ...
  return (
    <header className='bg-secondary text-white py-4 fixed w-full z-10'>
        {/* desktop navigation section */}
        <nav className='container flex justify-between items-center'>
            <div className='flex items-center gap-2'>
                <GiSteeringWheel size={35} className='text-primary'></GiSteeringWheel>
            ...
            <div className='md:hidden flex items-center'>
                { menu ? (
                    ...
                    <RiMenuUnfoldFill size={25} onClick={handleChange} ></RiMenuUnfoldFill>
                )}
            </div>
        </nav>
        ...
        <div className={`${menu ? "translate-x-0" : "-translate-x-full"} md:hidden flex flex-col absolute bg-secondary text-white left-0 top-16 font-semibold text-2xl text-center pt-8 pb-4 gap-8 w-3/4 h-fit rounded-br-xl transition-transform duration-300`}>
            ...
            <Link to="/services" className="hover:text-primary transition duration-200 ease-linear">Services</Link>
            <div>
                <button className='border-2 border-primary py-1 px-4 rounded-md'>Sign In</button>
            </div>
        </div>
    </header>
  )
